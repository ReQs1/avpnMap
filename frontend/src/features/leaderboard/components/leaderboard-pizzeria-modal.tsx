  ...
  return (
    ...
    >
      ...
      <div className="flex items-start gap-4 p-6 pb-4">
        <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-2xl bg-green-500 text-white shadow-sm">
          <Store size={32} />
        </div>
        <div className="flex flex-col items-start pt-1">
          ...
          <div className="mt-1 flex flex-wrap items-center gap-2 text-xs font-medium text-slate-500">
            {details?.memberNumber && (
              <>
                <span className="rounded bg-slate-100 px-1.5 py-0.5 tracking-wider text-slate-600 uppercase">
            ...
            <span className="flex items-center gap-1">
              <Globe size={14} className="text-slate-400" /> {pizzeria.nation}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-4">
        {isLoading ? (
          ...
          details?.address && (
            <div className="flex flex-col gap-2 rounded-xl border border-gray-100 bg-slate-50 p-3 text-sm text-slate-600 shadow-sm">
              ...
              {details.website && (
                <a
                  ...
                  className="ml-6 flex w-fit items-center gap-1 text-xs font-semibold text-red-500 transition-colors hover:text-red-600"
                >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col">
              {displayedReviews?.map((visit) => (
                <div
                  key={visit.id}
                  className="flex flex-col gap-1.5 border-b border-gray-100 py-4 last:border-0"
                >
              ...
              {details && details.visits?.length > 3 && (
                <button
                  ...
                  className="mt-2 flex w-full items-center justify-center gap-1 rounded-lg bg-slate-50 py-2.5 text-xs font-semibold text-slate-600 transition-colors hover:bg-slate-100"
                >
                  {showAllReviews ? (
                    <>
...
function StatBox({ value, label }: { value: React.ReactNode; label: string }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-xl border border-slate-100 bg-slate-50 py-3.5 text-center shadow-sm">
      <span className="mb-1 text-lg leading-none font-bold sm:text-xl">
        {value}
      </span>
...
function SkeletonRow() {
  return <div className="h-20 w-full animate-pulse rounded-xl bg-slate-100" />;
}
