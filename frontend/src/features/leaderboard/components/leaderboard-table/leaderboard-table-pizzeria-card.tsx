...
export default function LeaderboardTablePizzeriaCard({ pizzeria }: Props) {
  ...
  if (isFirst || isSecond || isThird) bgClass = "bg-green-50";
  else if (isEven) bgClass = "bg-gray-200/60";

  return (
    <button
      className={cn(
        "relative z-0 grid grid-cols-[32px_minmax(0,1fr)_50px] items-center gap-3 border-b border-gray-100 px-4 py-3 transition-colors last:border-none hover:bg-slate-200 focus-visible:z-10 focus-visible:bg-slate-200 focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:outline-none focus-visible:ring-inset sm:grid-cols-[48px_3fr_1fr_1fr_50px] sm:gap-4 md:grid-cols-[48px_1fr_1fr_1fr_50px]",
        bgClass,
      )}
    >
      {/* Position / Medal */}
      ...
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="flex w-full min-w-0 flex-col">
          ...
          <div className="mt-0.5 flex w-full min-w-0 items-center text-xs text-slate-500">
      ...
      {/* Score */}
      <div className="flex items-baseline justify-end gap-1 text-right text-lg font-bold text-green-600">
