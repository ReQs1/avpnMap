...
export default function LeaderboardTablePagination({
  ...
  return (
    <div className="mx-auto mt-4 flex w-full max-w-5xl flex-col items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white px-2 py-3 shadow-sm sm:flex-row sm:gap-4 sm:px-4">
      {/* Left side: Info */}
      ...
      <div className="flex flex-wrap items-center justify-center gap-1 sm:gap-2">
        {/* Prev Button */}
        {page <= 1 ? (
          <span className="px-1 py-1 text-xs font-medium text-slate-300 sm:px-2 sm:text-sm">
            Prev
          </span>
        ) : (
          <Link
            ...
            className={cn(
              "px-1 py-1 text-xs font-medium text-slate-500 transition-colors hover:text-slate-800 sm:px-2 sm:text-sm",
          ...
          {pages.map((p, index) => {
            if (p === "...") {
              return (
                <span
                  key={`ellipsis-${index}`}
                  className="flex h-7 w-7 items-center justify-center text-xs text-slate-400 sm:h-8 sm:w-8 sm:text-sm"
                >
                  ...
                </span>
              );
